...
const ManagementPage = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["admins"],
    queryFn: async () => {
      const res = await getAxiosInstance("admin").get("/get-all-admins");
  ...
  const { mutate: updateRole, isPending: updating } = useMutation({
    mutationFn: async () => {
      return await getAxiosInstance('admin').put("/add-new-admin", {
        email: search,
        role: selectedRole,
      });
    },
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex gap-8 pt-2">
                <button
                  type="button"
                  onClick={() => setOpen(false)}
                  className="w-full bg-slate-700 text-white px-4 py-2 rounded hover:bg-slate-700"
                >
                  Cancel
                </button>
                <button
