...
const fetchOrders = async () => {
  const res = await getAxiosInstance("order").get("/get-admin-orders");
  ...
  const columns = useMemo(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }: any) => (
          <span className="text-white">
            {row.original.shop?.name ?? "Unknown Shoop"}
          </span>
        ),
      },
      {
      ...
      },
      {
        ...
        cell: ({ row }: any) => {
          const sellerShare = row.original.total * 0.9; // Assuming 10% platform fee
          return (
            <span className="text-green-400">${sellerShare.toFixed(2)}</span>
          );
        },
      },
      {
